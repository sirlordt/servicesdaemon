...
/**
 * A reference implementation of a JDBC RowSet synchronization provider with the
 * ability to read and write rowsets in well formed XML using the standard
 * WebRowSet schema.
 * 
 * <h3>1.0 Background</h3> This synchronization provider is registered with the
 * <code>SyncFactory</code> by default as the
 * <code>com.sun.rowset.providers.RIXMLProvider</code>.
 * <P>
 * A <code>WebRowSet</code> object uses an <code>RIXMLProvider</code>
 * implementation to read an XML data source or to write itself in XML format
 * using the <code>WebRowSet</code> XML schema definition available at
 * 
 * <pre>
 ...
 * <h3>2.0 Usage</h3>
 * A <code>WebRowSet</code> implementation is created with an
 * <code>RIXMLProvider</code> by default.
 * 
 * <pre>
 ...
 * <code>RIOptimisticProvider</code> when no provider is specified, but the
 * implementation of the default constructor for <code>WebRowSet</code> sets the
 * provider to be the <code>RIXMLProvider</code> implementation. Therefore, the
 * following line of code is executed behind the scenes as part of the
...
public final class RIXMLProvider extends SyncProvider {
	 ...
	 */
	private String providerID = "rowset.providers.RIXMLProvider";
	...
	/**
	 * This provider is available to all JDBC <code>RowSet</code>
	 ...
	 * 
	 * @return a <code>String</code> object with the fully specified class name
	 *         of this <code>RIOptimisticProvider</code> implementation
	 */
	@Override
	...
	/**
	 * Retrieves the reader that this <code>WebRowSet</code> object will call
	 * when its <code>readXml</code> method is called.
	 * 
	...
	/**
	 * Retrieves the writer that this <code>WebRowSet</code> object will call
	 * when its <code>writeXml</code> method is called.
	 * 
	...
	/**
	 * Throws an unsupported operation exception as this method does function
	 * with non-locking XML data sources.
	 */
	@Override
	public void setDataSourceLock(int lock) throws SyncProviderException {
		throw new UnsupportedOperationException(resBundle.handleGetObject(
				"rixml.unsupp").toString());
	}

	/**
	 * Returns a null object as RowSetWriters are not returned by this
	 * SyncProvider
	 */
	@Override
