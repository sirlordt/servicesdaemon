...
public class CSystemDescribeObject extends CAbstractService {

    public final static String getJarFolder() {

        String name =  CSystemDescribeObject.class.getCanonicalName().replace( '.', '/' );

        String s = CSystemDescribeObject.class.getClass().getResource( "/" + name + ".class" ).toString();
	...
	public boolean InitializeService( CServicesDaemonConfig ServicesDaemonConfig, CAbstractConfigLoader OwnerConfig ) { // Alternate manual contructor
		...
		try {
			...
			DefaultConstantsSystemDescribeObject.strDefaultRunningPath = this.strJarRunningPath;
			this.strServiceName = "System.Describe.Object";
			this.strServiceVersion = "0.0.0.1";

			this.SetupService( DefaultConstantsSystemDescribeObject.strDefaultMainFileLog, DefaultConstantsSystemDescribeObject.strDefaultRunningPath + DefaultConstantsServices.strDefaultLangsDir + DefaultConstantsSystemDescribeObject.strDefaultMainFile + "." + ServicesDaemonConfig.strDefaultLang ); //Init the Logger and Lang
			...
			InputParameter = new CInputServiceParameter( ConstantsSystemDescribeObject._RequestObjectType, true, ConstantsSystemDescribeObject._RequestObjectTypeType, "0", TParameterScope.IN, ServiceLang.Translate( "Type of object to be verified its existence 1 = Table, 2 = View, 3 = Function, 4 = Stored Procedure" ) );

			ServiceInputParameters.add( InputParameter );

			InputParameter = new CInputServiceParameter( ConstantsSystemDescribeObject._RequestObjectName, true, ConstantsSystemDescribeObject._RequestObjectNameType, ConstantsSystemDescribeObject._RequestObjectNameTypeLength, TParameterScope.IN, ServiceLang.Translate( "Name of the object to be verified its existence" ) );
	...
	public int ExecuteService(int intEntryCode, HttpServletRequest Request, HttpServletResponse Response, String strSecurityTokenID, HashMap<String, CAbstractService> RegisteredServices, CAbstractResponseFormat ResponseFormat, String strResponseFormatVersion ) {
		...
		if ( this.CheckServiceInputParameters( GroupsInputParametersService.get( ConstantsServicesTags._Default ), Request, Response, ResponseFormat, strResponseFormatVersion, LocalConfigDBConnection!=null?LocalConfigDBConnection.strDateTimeFormat:OwnerConfig.getConfigValue( ConstantsSystemDescribeObject._Global_DateTime_Format ), LocalConfigDBConnection!=null?LocalConfigDBConnection.strDateFormat:OwnerConfig.getConfigValue( ConstantsSystemDescribeObject._Global_Date_Format ), LocalConfigDBConnection!=null?LocalConfigDBConnection.strTimeFormat:OwnerConfig.getConfigValue( ConstantsSystemDescribeObject._Global_Time_Format ), this.ServiceLogger!=null?this.ServiceLogger:this.OwnerLogger, this.ServiceLang!=null?this.ServiceLang:this.OwnerLang ) == true ) {
						...
						if ( LocalConfigDBConnection != null ) {
							...
							if ( DBEngine != null ) {
								...
								if ( DBConnection != null ) {
									...
									String strObjectType = Request.getParameter( ConstantsSystemDescribeObject._RequestObjectType );
									
									if ( strObjectType.equals( ConstantsSystemDescribeObject._Table ) || strObjectType.equals( ConstantsSystemDescribeObject._View ) || strObjectType.equals( ConstantsSystemDescribeObject._Function ) || strObjectType.equals( ConstantsSystemDescribeObject._Procedure ) ) {
									
										String strObjectName = Request.getParameter( ConstantsSystemDescribeObject._RequestObjectName );
