...
public class CSystemListObjects extends CAbstractService {

    public final static String getJarFolder() {

        String name =  CSystemListObjects.class.getCanonicalName().replace( '.', '/' );

        String s = CSystemListObjects.class.getClass().getResource( "/" + name + ".class" ).toString();
	...
	public boolean InitializeService( CServicesDaemonConfig ServicesDaemonConfig, CAbstractConfigLoader OwnerConfig ) { // Alternate manual contructor
		...
		try {
			...
			DefaultConstantsSystemListObjects.strDefaultRunningPath = this.strJarRunningPath;
			this.strServiceName = "System.List.Objects";
			this.strServiceVersion = "0.0.0.1";

			this.SetupService( DefaultConstantsSystemListObjects.strDefaultMainFileLog, DefaultConstantsSystemListObjects.strDefaultRunningPath + DefaultConstantsServices.strDefaultLangsDir + DefaultConstantsSystemListObjects.strDefaultMainFile + "." + ServicesDaemonConfig.strDefaultLang ); //Init the Logger and Lang
			...
			this.strServiceDescription = ServiceLang.Translate( "List all objects (table, view, procedure, function) in the database" );
			...
			InputParameter = new CInputServiceParameter( ConstantsSystemListObjects._RequestObjectType, true, ConstantsSystemListObjects._RequestObjectTypeType, "0", TParameterScope.IN, ServiceLang.Translate( "Type of object to list 1 = Table, 2 = View, 3 = Function, 4 = Stored Procedure" ) );
	...
	public int ExecuteService(int intEntryCode, HttpServletRequest Request, HttpServletResponse Response, String strSecurityTokenID, HashMap<String, CAbstractService> RegisteredServices, CAbstractResponseFormat ResponseFormat, String strResponseFormatVersion ) {
		...
		if ( this.CheckServiceInputParameters( GroupsInputParametersService.get( ConstantsServicesTags._Default ), Request, Response, ResponseFormat, strResponseFormatVersion, LocalConfigDBConnection!=null?LocalConfigDBConnection.strDateTimeFormat:OwnerConfig.getConfigValue( ConstantsSystemListObjects._Global_DateTime_Format ), LocalConfigDBConnection!=null?LocalConfigDBConnection.strDateFormat:OwnerConfig.getConfigValue( ConstantsSystemListObjects._Global_Date_Format ), LocalConfigDBConnection!=null?LocalConfigDBConnection.strTimeFormat:OwnerConfig.getConfigValue( ConstantsSystemListObjects._Global_Time_Format ), this.ServiceLogger!=null?this.ServiceLogger:this.OwnerLogger, this.ServiceLang!=null?this.ServiceLang:this.OwnerLang ) == true ) {
						...
						if ( LocalConfigDBConnection != null ) {
							...
							if ( DBEngine != null ) {
								...
								if ( DBConnection != null ) {
									...
									String strObjectType = Request.getParameter( ConstantsSystemListObjects._RequestObjectType );
